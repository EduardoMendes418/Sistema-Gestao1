	...
	<form method="post" enctype="multipart/form-data">
		<?php
			if(isset($_POST['acao'])){
				...
				if($titulo == '' || $conteudo == ''){
					Painel::alert('erro',' Campos vazios não são permitos');
				}else if ($capa['tmp_name'] == ''){
					Painel::alert('erro',' A imagem de capa precisa ser selecionado');
				}else{
					if(Painel::imagemValida($capa)){
						...
						$slong = Painel::generateSlong($titulo);
						$arr = ['categoria_id'=>$categoria_id,'titulo'=>$titulo,'conteudo'=>$conteudo,'capa'=>$imagem,'order_id'=>'0','slong'=>$slong,'date'=>date('Y-m-d'),'nome_tabela'=>'tb_site.noticias'];
					if(Painel::insert($arr)){
						Painel::redirect(INCLUDE_PATH_PAINEL.'cadastrarnoticias?sucesso');
					}
